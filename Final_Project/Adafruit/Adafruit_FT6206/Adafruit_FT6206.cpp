...
void Adafruit_FT6206::readData(uint16_t *x, uint16_t *y) {
  ...
  i2cdata[0] = FT6206_ADDR<<1;
  i2cdata[1] = 0;

  Write(htouch, &i2cdata, &length);
  
  length = 17;
  Read(htouch, &i2cdata, &length);
  
  for(int i=0; i<16; i++ ){
    i2cdat[i] = i2cdata[i+1];
  }
...
uint8_t Adafruit_FT6206::readRegister8(uint8_t reg) {
  ...
  i2crreg[0] = FT6206_ADDR<<1;
  i2crreg[1] = reg;

  Write(htouch, &i2crreg, &length);
  
  // Wire.requestFrom((uint8_t)FT6206_ADDR, (uint8_t)1);
  
  Read(htouch, &i2crreg, &length);
...
void Adafruit_FT6206::writeRegister8(uint8_t reg, uint8_t val) {
  ...
  i2cwrite[0] = FT6206_ADDR<<1;
  i2cwrite[1] = reg;
  i2cwrite[2] = val;
  
  Write(htouch, &i2cwrite, &length);
}
