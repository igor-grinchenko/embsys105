/*
    ...
    2020/3 Igor Grinchenko updated MP3 stream function
*/
...
void Mp3Stream(HANDLE hMp3, INT8U *pBuf, INT32U bufLen)
{
    ...
    InputCommandEnum *streamCommand;
    
    streamCommand = (InputCommandEnum*)OSMboxAccept(commandMsg);
    
    Mp3StreamInit(hMp3);
    
    chunkLen = MP3_DECODER_BUF_SIZE;

      while (!done && ((*streamCommand==INPUTCOMMAND_PLAY)|| (*streamCommand==INPUTCOMMAND_PAUSE)))
      {
        while (*streamCommand==INPUTCOMMAND_PAUSE)
        {
          OSTimeDly(50);
        }
